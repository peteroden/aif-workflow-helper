...
@pytest.mark.asyncio
async def test_upload_rejects_missing_model(agents_client_mock, caplog):
    agent_dict = {"name": "test-agent", "instructions": "Do things"}
    result = await create_or_update_agent(agent_dict, agents_client_mock)
    assert result is None
    assert any("missing required 'model'" in m for m in caplog.messages)

@pytest.mark.asyncio
async def test_upload_rejects_missing_instructions(agents_client_mock, caplog):
...
@pytest.mark.asyncio
async def test_unresolved_connected_agent_logs_warning(agents_client_mock, caplog):
    agent_dict = {
        "name": "parent",
        "model": "gpt-4",
        "instructions": "Parent",
    ...
    await create_or_update_agent(agent_dict, agents_client_mock)
...
@pytest.mark.asyncio
async def test_circular_dependency_raises(agents_client_mock):
    ...
    with pytest.raises(ValueError):
        await create_or_update_agents(agents, agents_client_mock)
